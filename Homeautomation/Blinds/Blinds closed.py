## Blinds closed.py script by Edward George www.Etechprojects.wordpress.com ##
...
try:
    ...
    closeTime = config.get('blinds', 'close time(secs)') # -> "closeTime"
    print 'close time ' + closeTime + ' seconds'
    closeTime = float(closeTime)
    Gpio = config.get('blinds', 'gpio pin close') # -> "Gpio"
    print 'Gpio pin ' + Gpio
    ...
    if state == 'open':
        ...
        config.set('blinds', 'state', 'closed')
        print ''
        print 'blinds now closed'
